...
class JobApplicationService {
  async createJobApplication(applicationData, cvFilename) {
    ...
    emailService
      .sendJobApplicationAdminNotification(application)
      .catch((err) =>
        console.error(
          "Job application admin notification failed:",
          err.message,
        ),
      );
  ...
  async getJobApplicationStats() {
    ...
    const reviewed = await prisma.jobApplication.count({
    ...
    const unread = await prisma.jobApplication.count({
      where: { isRead: false },
    });
