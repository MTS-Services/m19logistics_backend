...
router.post(
  ...
  [
    ...
    body('timeSlot').isIn(['AM', 'PM', 'SAME_DAY']).withMessage('Valid time slot is required'),
    ...
    body('requestedBy').trim().notEmpty().withMessage('Requested by field is required'),
...
router.get(
  ...
  authorize('CUSTOMER'),
  [
    query('status').optional().isIn(['ALL', 'RECEIVED', 'ALLOCATED', 'DELIVERED', 'CANCELLED']),
